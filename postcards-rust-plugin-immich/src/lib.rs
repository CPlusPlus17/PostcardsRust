...
impl ICommand for ImmichCommand {
    ...
    async fn sync(&mut self) -> Result<()> {
        ...
        for asset in remote_assets {
            ...
            let safe_name = safe_filename(&asset.original_file_name);
            let local_file_name = format!("{}_{}", asset.id, safe_name);
            let local_path = self.config.media_folder.join(&local_file_name);

            if already_cached && local_path.exists() {
                continue;
            }
    ...
    async fn get_next_photo(&self) -> Result<PathBuf> {
        ...
        // Return the first photo in the manifest (sorted oldest first) that exists on disk
        ...
        bail!(
            "No cached photos currently exist on disk in {:?}",
            self.config.media_folder
        );
    }
        ...
        let server = tokio::spawn(async move {
            for _ in 0..10 {
                ...
                if req.contains("GET /api/server/version") {
                    ...
                    let resp = format!("HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\n\r\n{}", body.len(), body);
                    let _ = socket.write_all(resp.as_bytes()).await;
                } else if req.contains("GET /api/assets/asset-uuid-1/original") {
                    let body = b"fake-jpeg-binary-image-data-here";
